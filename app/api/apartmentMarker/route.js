    ...
    const apartmentMarkers = await prisma.Apartment_Marker.findMany({
      include: {
        details: {
          select: {
            address: true,
            complexNamePublic: true,
          },
        },
      },
    });

    const processedMarkers = apartmentMarkers.map((marker) => {
      ...
      return {
        ...marker,
        complexName:
          marker.details?.complexNamePublic || marker.complexUniqueId,
        dong,
        address,
      };
    });

    await redis.set(CACHE_KEY, JSON.stringify(processedMarkers), {
      EX: CACHE_DURATION,
    });

    return NextResponse.json(processedMarkers, {
      status: 200,
      headers: {
        "Cache-Control": "public, max-age=300",
      },
...
export async function POST(request) {
  try {
    ...
    const { complexUniqueId, latitude, longitude } = body;
    ...
    return NextResponse.json(
      { error: "데이터 추가 실패", details: error.message },
      { status: 500 }
    );
  }
}
